...
async def api_agent(req: AgentReq):
    ...
    prompt = f"""당신은 SEO/AEO/GEO 검색 노출 분석 전문 Agent입니다. 한국어로 답변하세요.

사용자가 제공한 콘텐츠를 분석하고, 아래 항목들을 JSON 형태로 반환하세요:

1. **keyword_analysis**: 이 콘텐츠에서 추출한 핵심 키워드 5개와 각각의 검색 경쟁력 (상/중/하)
2. **serp_prediction**: 이 콘텐츠가 Google/네이버에서 어떤 검색어로 노출될 가능성이 높은지 예측 (5개)
3. **ai_citation_analysis**: 각 AI 플랫폼(ChatGPT, Perplexity, Google AI, 네이버 AI, Claude)이 이 콘텐츠를 인용할 때 어떤 부분을 발췌할 가능성이 높은지 구체적 문장 지적
4. **competitor_gaps**: 이 주제에서 경쟁 콘텐츠가 일반적으로 다루지만 이 글에는 빠진 내용
5. **action_plan**: 노출을 극대화하기 위한 즉시 실행 가능한 3단계 액션 플랜

{"URL: " + req.url if req.url else ""}
{"사용자 지정 키워드: " + ", ".join(req.keywords) if req.keywords else ""}
...
반드시 한국어로 작성하되, 구체적이고 실행 가능한 조언을 주세요. 각 섹션에 명확한 제목을 붙이세요."""
