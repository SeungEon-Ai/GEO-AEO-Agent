...
@app.post("/api/agent")
async def api_agent(req: AgentReq):
    """Gemini Agent: 콘텐츠의 검색 노출 가능성을 분석"""
    if not GEMINI_KEY:
        raise HTTPException(500, "GEMINI_API_KEY 환경변수가 설정되지 않았습니다")

    prompt = f"""당신은 SEO/AEO/GEO 검색 노출 분석 전문 Agent입니다.

아래 콘텐츠를 분석해서 **반드시 한국어로, 아래 정해진 5개 섹션 형식**으로 답변하세요.
절대 JSON이나 코드 블록으로 답하지 마세요. 사람이 바로 읽을 수 있는 깔끔한 텍스트로 작성하세요.

---

📌 섹션 1: 핵심 키워드 분석
- 이 콘텐츠에서 추출한 핵심 키워드 5개를 표로 정리
- 각 키워드의 검색 경쟁력(🔴높음 🟡중간 🟢낮음)과 추천 롱테일 키워드를 함께 제시
- 형식: "키워드 | 경쟁력 | 추천 롱테일"

📌 섹션 2: 검색 노출 예측
- Google에서 노출될 가능성이 높은 검색어 3개
- 네이버에서 노출될 가능성이 높은 검색어 3개
- 각각 왜 그 검색어에 노출될 수 있는지 한 줄 이유 설명

📌 섹션 3: AI 플랫폼별 인용 예상
- ChatGPT, Perplexity, Google AI Overview, 네이버 AI 브리핑, Claude 각각이 이 콘텐츠에서 인용할 가능성이 높은 구체적 문장이나 단락을 지적
- 왜 그 부분이 인용될지 이유도 함께

📌 섹션 4: 경쟁 콘텐츠 대비 빠진 내용
- 같은 주제의 상위 랭킹 콘텐츠가 보통 다루지만, 이 글에 빠져있는 내용 3~5가지

📌 섹션 5: 즉시 실행 액션 플랜
- 지금 바로 할 수 있는 3단계 개선 계획
- 각 단계별로 구체적 작업과 예상 효과를 명시

---

{"🔗 URL: " + req.url if req.url else ""}
{"🎯 타겟 키워드: " + ", ".join(req.keywords) if req.keywords else ""}
📂 콘텐츠 유형: {req.content_type}
...
==="""
    ...
    return {"analysis": reply or "분석 결과를 받지 못했습니다."}
